<script lang="ts">
  ...
  function containerKey(e: KeyboardEvent) {
...
<div class="k-dialog" role="dialog" aria-modal="true" aria-label={title} tabindex="-1" onkeydown={containerKey}>
  <div class="k-dialog__head">{title}</div>
  <input class="k-input" {placeholder} bind:value={draft} bind:this={inputEl} aria-label={title} onkeydown={inputKey} />
