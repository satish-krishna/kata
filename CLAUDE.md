...
A run's permission posture is explicit and never inferred. `[permissions] mode` is `bypass` by default (`--dangerously-skip-permissions`, what every run did before the field existed); `prompt` swaps that flag for `--permission-prompt-tool mcp__kata-ask__approve_tool`, which is the route on a machine whose managed settings set `permissions.disableBypassPermissionsMode`. **There is deliberately no automatic fallback between the two** — a run whose safety posture changed silently between machines is not reproducible.
