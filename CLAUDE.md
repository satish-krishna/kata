...
Rule matching happens engine-side (`permission::decide`, deny before allow); the MCP server only bridges. `permissions.unmatched = "ask"` requires `[interactive] enabled = true` and `validate` rejects the combination otherwise, since nothing else could answer.
