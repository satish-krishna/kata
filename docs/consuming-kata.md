...
| `run.diff` | `worktree?`, `branch?`, `files[]`, `insertions`, `deletions` | Changeset summary, emitted just before the terminal event on every run. worktree/branch present only under worktree isolation. |
...
Then a note on the terminal events: `run.error` and `run.cancelled` now carry `cost_usd` and `duration_ms` matching `run.completed`. `cost_usd` is `null` when the leash kills the child before claude reports a final cost (timeout, cancel, turn cap) and is present on the budget path (exit 122); `duration_ms` is always present.
