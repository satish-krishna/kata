...
- [x] **Workbench polish + marketplace plugin discovery (v0.2.0).** A `Library` link in the toolbar; the model field became an alias selector — `default`/`opus`/`sonnet`/`haiku` (resolved per tier at run time, honoring `ANTHROPIC_DEFAULT_*_MODEL`) plus a free-text custom pin — replacing the hard-coded id list that went stale each model release; the library's run-detail actions explain via tooltip why they disable when a run's kata is no longer saved; the `<a>`-as-button hover bug (azure label on the azure hover background) fixed in both the app and `design/` stylesheets. Engine: `build_invocation` omits `--model` for a blank/whitespace id (no more `--model ""`), and the catalog now discovers marketplace-installed plugins from `~/.claude/plugins/installed_plugins.json` (incl. the modern `.claude-plugin/plugin.json` manifest location), so plugins like superpowers become selectable. Adds a Svelte component-test harness (Testing Library + jsdom) and an `npm run screenshots` release-capture script. **Status:** merged to `main` (#19); cut as [v0.2.0](https://github.com/satish-krishna/kata/releases/tag/v0.2.0) with the Workbench screenshots attached.
...
- [~] **Per-run environment override on `RunSpec`.** `env` (set/override, name → literal value) and `env_remove` (unset) applied to the `claude` child `Command` only — never via host-process mutation — so an in-process host can give two concurrent runs divergent egress (e.g. direct-to-Anthropic vs bring-your-own-proxy) without a process-global data race. Resolution order: inherited → plugin forwards → `token_env`-derived `ANTHROPIC_API_KEY` → `env` → `env_remove` (removal wins); `validate` enforces disjointness, well-formed names, and engine-reserved names. Additive and backward-compatible (schema stays 1). Design: `docs/superpowers/specs/2026-07-03-kata-runspec-env-override-design.md`. **Status:** PR [#32](https://github.com/satish-krishna/kata/pull/32).
  - [ ] Deferred follow-up: `kata run` CLI flags for ad-hoc env injection. Spec-only today; add when an out-of-process consumer needs it, reusing the same `validate` rules rather than reinventing them.
...
- [x] **CI (GitHub Actions):** `.github/workflows/ci.yml` runs the green-before-merge gate on push/PR to `main`. Rust job (windows-latest, matching the release platform: `cargo fmt --all --check`, `cargo clippy --all-targets -- -D warnings`, `cargo test --workspace`, `cargo build --locked`, with the `kata` sidecar staged so the Tauri backend compiles); web job (ubuntu-latest: `npm ci`, `npm run check`, `npm test`). The real-claude smoke test stays opt-in (`KATA_SMOKE_REAL` unset → skipped). **Status:** merged to `main` (#25); a secret-gated logged-in runner for the smoke test is still optional/open.
- [~] **Release / packaging:** local Windows release process in place — `scripts/bump-version.ps1` + `scripts/build-release.ps1` build the standalone `kata` CLI and the Workbench NSIS/MSI installers; tag `vX.Y.Z` and `gh release create` by hand (see `docs/releasing.md`). Releases cut: `v0.1.0`, `v0.2.0` (the latter with Workbench screenshots attached, captured via `app: npm run screenshots`). Still open: crates.io publish vs. tagged binaries, macOS/Linux artifacts, code-signing/CI. MIT.
